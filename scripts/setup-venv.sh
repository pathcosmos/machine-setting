...
echo "  Hardware: GPU=$HAS_GPU CUDA=$CUDA_SUFFIX"
...
# --- GPU/CPU packages ---
echo ""
if [ "$HAS_GPU" = true ] && [ -f "$PKG_DIR/requirements-gpu.txt" ]; then
    # Get index URL for this CUDA version
    INDEX_URL=""
...
"$VENV_PATH/bin/python" -c "import torch; print(f'    torch {torch.__version__} (CUDA: {torch.cuda.is_available()})')" 2>/dev/null || echo "    torch: not installed"
