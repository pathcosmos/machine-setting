...
### cuda-defense-check.sh — CUDA 프로세스 레벨 진단

PyTorch/Ollama GPU 공존 환경에서 프로세스 레벨 CUDA 상태를 점검합니다:
1. CUDA 프로세스 상태 (좀비, hang 감지)
2. Ollama 서비스 상태
3. GPU 메모리 압력
4. CUDA 드라이버 일관성

```bash
./scripts/cuda-defense-check.sh           # 전체 진단
./scripts/cuda-defense-check.sh --summary # 한 줄 요약
./scripts/cuda-defense-check.sh --json    # 자동화용 JSON
```
